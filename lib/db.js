...
export function formatArticleRow(row) {
  ...
  return {
    ...
    audioUrlTa: row.audio_url_ta || null,
    audioUrlEn: row.audio_url_en || null,
    audio_url_ta: row.audio_url_ta || null,
...
/**
 * Like / Unlike a comment by ID (toggles count, prevents negative likes).
 */
export async function likeArticleComment(id, isUnlike = false) {
  const pgPool = getPgPool();
  if (!pgPool) throw new Error('Database connection not available');

  const deltaSql = isUnlike 
    ? 'GREATEST(0, COALESCE(likes_count, 0) - 1)' 
    : 'COALESCE(likes_count, 0) + 1';

  const res = await pgPool.query(
    `UPDATE article_comments
     SET likes_count = ${deltaSql}
     WHERE id = $1
