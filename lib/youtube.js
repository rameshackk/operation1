...
export async function resolvePublisherYouTubeInput(input, apiKey) {
  ...
  if (!res.ok) {
    console.warn(`[YouTube Resolver] YouTube API handle lookup note (${res.status}): returning heuristic handle fallback`);
    const cleanTitle = handle.replace(/^@/, '');
    ...
    channelCache.set(handle, fallbackResult);
