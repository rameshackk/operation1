...
export async function resolvePublisherYouTubeInput(input, apiKey) {
  ...
  if (!res.ok) {
    const errorText = await res.text();
    throw new Error(`YouTube channels.list for handle failed (${res.status}): ${errorText}`);
  }
  ...
  if (!item) {
    throw new Error(`Could not find YouTube channel for handle: ${handle}. Please ensure the handle is spelled correctly.`);
  }
