...
function LoginReminderModal({ currentHash, onNavigate }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-sm animate-fade-in">
      <div 
        className="relative w-full max-w-md bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-7 border border-amber-500/30 shadow-2xl shadow-amber-500/10 text-slate-900 dark:text-slate-100 overflow-hidden modal-card-unified"
        role="dialog"
        aria-modal="true"
      >
        {/* Ambient Glow */}
        ...
        <div className="flex items-center gap-3 mb-4">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-amber-500 via-amber-600 to-amber-800 flex items-center justify-center text-white shadow-lg shadow-amber-500/30 shrink-0 border border-amber-400/40">
            <svg className="w-6 h-6 animate-bounce" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
            </svg>
          </div>
          <div>
            <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 text-[10px] font-extrabold uppercase tracking-wider rounded-full bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/30">
              <span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse" />
              {isTa ? 'தினசரி செய்திகள்' : 'Daily Updates'}
            </span>
            <h3 className="text-lg sm:text-xl font-black tracking-tight text-slate-900 dark:text-white mt-1">
              {isTa ? 'தினசரி புதுப்பிப்புகளுக்கு உள்நுழைக!' : 'Login for Daily Updates!'}
            </h3>
          </div>
        </div>
        ...
        <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-300 leading-relaxed mb-5">
          {isTa
            ? 'முதலீட்டு திசையின் பிரத்யேக மியூச்சுவல் ஃபண்ட் பகுப்பாய்வுகள், பங்குச் சந்தை நேரடிச் செய்திகள் மற்றும் நிபுணர் வழிகாட்டல்களை உடனுக்குடன் பெற இப்போதே உள்நுழையுங்கள்.'
            : 'Get real-time mutual fund recommendations, stock market alerts, and exclusive research articles delivered to you daily by signing in.'}
        </p>

        {/* Benefits List */}
        <div className="space-y-2 mb-6 bg-slate-50 dark:bg-slate-800/60 p-3.5 rounded-2xl border border-slate-200/80 dark:border-slate-700/60">
          <div className="flex items-center gap-2.5 text-xs text-slate-700 dark:text-slate-200 font-semibold">
            <span className="text-emerald-500 font-bold text-sm">✓</span>
            <span>{isTa ? 'மியூச்சுவல் ஃபண்ட் & SIP முதலீட்டு வழிகாட்டல்கள்' : 'Mutual Fund & SIP Strategy Guides'}</span>
          </div>
          <div className="flex items-center gap-2.5 text-xs text-slate-700 dark:text-slate-200 font-semibold">
            <span className="text-emerald-500 font-bold text-sm">✓</span>
            <span>{isTa ? 'சரிபார்க்கப்பட்ட நிதி ஆலோசகர்களின் ஆய்வுக் கட்டுரைகள்' : 'Verified Financial Advisor Insights'}</span>
          </div>
          <div className="flex items-center gap-2.5 text-xs text-slate-700 dark:text-slate-200 font-semibold">
            <span className="text-emerald-500 font-bold text-sm">✓</span>
            <span>{isTa ? 'நேரலை சந்தை எச்சரிக்கைகள் & உடனடி நிதிச் செய்திகள்' : 'Real-time Market Alerts & Breaking Wealth News'}</span>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2.5">
          <button
            ...
            className="w-full py-3 px-4 rounded-xl bg-gradient-to-r from-amber-600 to-amber-500 hover:from-amber-500 hover:to-amber-400 text-white font-black text-sm shadow-lg shadow-amber-500/25 hover:shadow-amber-500/40 hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-2"
          >
            <span>{isTa ? 'இப்போதே உள்நுழைக' : 'Sign In Now'}</span>
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex items-center justify-between pt-1">
            ...
            >
              {isTa ? 'புதியவரா? கணக்கு தொடங்குக' : 'New here? Create Account'}
            </button>
            <button
              onClick={handleDismiss}
              className="text-xs text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 font-semibold"
            >
              {isTa ? 'பின்னர் பார்க்கலாம்' : 'Maybe Later'}
            </button>
          </div>
        </div>
      </div>
