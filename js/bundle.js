...
function HeroSection({ news = newsData, onNavigate }) {
  ...
  const renderFeaturedTrack = (keyPrefix) => (
    <div key={keyPrefix} className="flex items-stretch gap-5 shrink-0 py-2">
        ...
        return (
          ...
          >
            ...
            <div className="relative aspect-[16/9] w-full overflow-hidden bg-slate-950">
              ...
              {/* Category Badge */}
              <span className="absolute top-3 left-3 px-2.5 py-0.5 text-[9px] sm:text-[10px] font-black uppercase tracking-wider rounded-lg bg-amber-500 text-slate-950 shadow-md">
                {(item.category || 'FINANCE').replace('-', ' ')}
              </span>

              {/* Live Tag / Date Badge */}
              <span className="absolute bottom-2.5 right-3 px-2 py-0.5 rounded-md bg-slate-950/85 text-slate-200 text-[10px] font-mono font-bold backdrop-blur-sm border border-white/10">
                📅 {formattedDate}
              </span>
            </div>
            ...
            <div className="p-4 sm:p-5 flex-1 flex flex-col justify-between gap-3">
              <div className="space-y-1.5">
                <h3 className="text-sm sm:text-base font-bold text-slate-900 dark:text-white line-clamp-2 font-serif group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors leading-snug">
                  {headline}
                </h3>
                {summary && (
              ...
              {/* Footer CTA Line */}
              <div className="flex items-center justify-between text-xs text-amber-600 dark:text-amber-400 font-bold border-t border-slate-100 dark:border-slate-800/80 pt-2.5">
    ...
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 space-y-3">
      <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
        ...
        <div className="text-[11px] text-slate-500 font-semibold flex items-center gap-2 font-mono">
          ...
          <span>⚡ LIVE NEWS TICKER • HOVER TO PAUSE</span>
        </div>
      </div>

      {/* Featured News Auto-Scrolling Ticker (Images + Content) */}
      <div className="featured-marquee-wrapper overflow-hidden py-1">
        <div className="animate-featured-marquee flex items-stretch gap-5 whitespace-normal">
